...
export default function Dropdown({ title, list }) {
  ...
  return (
    <div className='relative h-full w-36 flex items-center justify-start border-solid  p-2 cursor-pointer z-10' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <button className="hover:text-essencial-blue h-full w-full " onClick={() => setClick(true)}>{title}</button>
      {dropdown &&
        <div
          onClick={handleClick}
          className='absolute flex flex-col items-start justify-between w-52 top-16 -left-6 p-4 h-60 shadow-2xl bg-white'
        >
